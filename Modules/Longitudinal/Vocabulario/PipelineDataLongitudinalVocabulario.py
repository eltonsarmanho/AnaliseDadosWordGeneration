...
# -*- coding: utf-8 -*-
"""
PIPELINE DE DADOS LONGITUDINAIS - VOCABULÁRIO WORDGEN
Análise longitudinal do Vocabulário ao longo das Fases 2, 3 e 4
...
CSV_LONGITUDINAL_VOCABULARIO = LONGITUDINAL_DIR / "dados_longitudinais_Vocabulario.csv"
JSON_RESUMO_LONGITUDINAL = LONGITUDINAL_DIR / "resumo_longitudinal_Vocabulario.json"

class PipelineDataLongitudinalVocabulario:
    """Pipeline para processamento de dados longitudinais do Vocabulário"""
    ...
    def normalizar_dados_fase(self, df_pre: pd.DataFrame, df_pos: pd.DataFrame, fase: int) -> pd.DataFrame:
        ...
        for _, row_pre in df_pre.iterrows():
            try:
                # Calcular total PRE (apenas respostas válidas 'D' para vocabulário)
                respostas_pre = []
                for col in colunas_questoes_pre:
                    if col in row_pre and pd.notna(row_pre[col]):
                        valor = str(row_pre[col]).strip().upper()
                        if valor == 'D':  # Apenas contar respostas corretas
                            respostas_pre.append(1)
                        elif valor in ['A', 'B', 'C']:  # Respostas incorretas
                            respostas_pre.append(0)
                ...
                if not pos_match.empty:
                    ...
                    for col in colunas_questoes_pos:
                        if col in row_pos and pd.notna(row_pos[col]):
                            valor = str(row_pos[col]).strip().upper()
                            if valor == 'D':  # Apenas contar respostas corretas
                                respostas_pos.append(1)
                            elif valor in ['A', 'B', 'C']:  # Respostas incorretas
                                respostas_pos.append(0)
    ...
    def processar_todas_fases(self):
        ...
        print("🔄 Iniciando processamento longitudinal Vocabulário...")
    ...
    def salvar_dados(self):
        ...
        if self.dados_consolidados:
            ...
            df_consolidado.to_csv(CSV_LONGITUDINAL_VOCABULARIO, index=False, encoding='utf-8')
            print(f"✅ CSV salvo: {CSV_LONGITUDINAL_VOCABULARIO}")
    ...
    def executar_pipeline(self):
        ...
        print("🚀 Iniciando Pipeline Longitudinal Vocabulário")
        print("="*50)
        ...
        print("\n✅ Pipeline Longitudinal Vocabulário concluído!")
        return True

def main():
    """Função principal"""
    pipeline = PipelineDataLongitudinalVocabulario()
