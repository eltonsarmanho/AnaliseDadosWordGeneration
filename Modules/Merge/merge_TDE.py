...
# -*- coding: utf-8 -*-
"""
MERGE TDE - FASES 2, 3 e 4
Script para consolidar dados de TDE das três fases em um único CSV
...
# Arquivo de saída
ARQUIVO_SAIDA = DATA_DIR / "TDE_consolidado_fases_2_3_4.csv"
...
def carregar_e_processar_fase(fase: int, arquivo_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Colunas das questões (Q01 a Q40) - extrair apenas Pre e Pos, sem Delta
        colunas_questoes = []
        for i in range(1, 41):  # Q1 a Q40 para TDE
            
            # Procurar pelas colunas Pre e Pos (formato P01_Pre_, P01_Pos_)
            col_pre = None
            col_pos = None
            
            for col in df.columns:
                if col.startswith(f"P{i:02d}_Pre_"):
                    col_pre = col
                elif col.startswith(f"P{i:02d}_Pos_"):
                    col_pos = col
...
def main():
    """Função principal para consolidar dados de TDE"""
    print("=" * 70)
    print("🎯 CONSOLIDAÇÃO TDE - FASES 2, 3 e 4")
    print("=" * 70)
    ...
    for i in range(1, 41):  # TDE tem 40 questões
        q_pre = f"Q{i}_Pre"
        q_pos = f"Q{i}_Pos"
    ...
    print(f"Colunas de questões: Q1_Pre/Pos até Q40_Pre/Pos")
